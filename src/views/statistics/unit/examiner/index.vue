<template>
  <div class="examiner-statistics-page-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">单位体检人员统计</span>
...
import { examinerStatisticsList, unitGroupInfo, examinerLeadout } from '@/api/workload'
import moment from 'moment'

export default {
  name: 'examinerStatistics',
  data () {
    return {
      ...
      comboPrice: '', // 合计字段：套餐费用
      addPrice: '', // 合计字段：加项费用
      shouldPrice: '' // 合计字段： 应收费用
    }
  },
  watch: {
    screenHeight (val) {
  ...
  methods: {
    ...
    getList () {
      ...
      this.isLoading = true
      examinerStatisticsList(this.form).then(({ data }) => {
    ...
    async leadoutExcelHandle () {
      ...
      params.unitPersonnelResultList = this.tableData
      params.comboMoney = this.comboPrice
      params.addMoney = this.addPrice
      params.shouldMoney = this.shouldPrice

      this.leadoutLoading = true
      try {
        const { data } = await examinerLeadout(params)
        this.leadoutLoading = false
        let blob = new Blob([data], { type: "application/vnd.ms-excel" });
        let link = document.createElement("a");
        link.href = window.URL.createObjectURL(blob);
        link.download = "单位体检人员统计.xls";
        link.click();
      ...
      return [
        columns.map((column, columnIndex) => {
          ...
          switch (column.property) {
            case 'packagePrice':
              this.comboPrice = Math.round(XEUtils.sum(data, 'packagePrice') * 100) / 100
              return `${Math.round(XEUtils.sum(data, 'packagePrice') * 100) / 100}`
            case 'addItemPrice':
              this.addPrice = Math.round(XEUtils.sum(data, 'addItemPrice') * 100) / 100
              return `${Math.round(XEUtils.sum(data, 'addItemPrice') * 100) / 100}`
            case 'receivablePrice':
              this.shouldPrice = Math.round(XEUtils.sum(data, 'receivablePrice') * 100) / 100
              return `${Math.round(XEUtils.sum(data, 'receivablePrice') * 100) / 100}`
          }
          return '-'
        })
      ]
...
<style lang="less">
.examiner-statistics-page-wrapper {
  width: 100%;
  min-width: 1300px;
  height: inherit;
