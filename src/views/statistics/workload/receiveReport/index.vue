<template>
  <div class="receive-report-page-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">报告领取工作量统计</span>
...
import { receiveReportWorkloadList, unitGroupInfo, receiveReportLeadout } from '@/api/workload'
import moment from 'moment'

export default {
  name: 'receiveReportWorkload',
  data () {
    return {
  ...
  methods: {
    ...
    getList () {
      ...
      this.isLoading = true
      receiveReportWorkloadList(this.form).then(({ data }) => {
    ...
    async leadoutExcelHandle () {
      ...
      try {
        const { data } = await receiveReportLeadout(params)
        this.leadoutLoading = false
        let blob = new Blob([data], { type: "application/vnd.ms-excel" });
        let link = document.createElement("a");
        link.href = window.URL.createObjectURL(blob);
        link.download = "报告领取工作量统计.xlsx";
        link.click();
      ...
      return [
        columns.map((column, columnIndex) => {
          ...
          switch (column.property) {
            case 'number':
              return `累计报告领取数：${Math.round(XEUtils.sum(data, 'number') * 100) / 100}`
          }
          return '-'
        })
      ]
