<template>
  <div class="main-doctor-workload-page-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">主检医生工作量统计</span>
      ...
      <div class="page-body clearfix">
        ...
        <div class="table-wrapper">
          <div class="search-info">符合查询条件主检医生人数共：<span>{{tableData.length}}</span>人，工作量分析如下：</div>
...
import { mainDoctorWorkloadList, unitGroupInfo, mainDoctorLeadout } from '@/api/workload'
import moment from 'moment'

export default {
  name: 'mainDoctorWorkload',
  data () {
    return {
  ...
  methods: {
    ...
    getList () {
      ...
      this.isLoading = true
      mainDoctorWorkloadList(this.form).then(({ data }) => {
    ...
    async leadoutExcelHandle () {
      ...
      try {
        const { data } = await mainDoctorLeadout(params)
        this.leadoutLoading = false
        let blob = new Blob([data], { type: "application/vnd.ms-excel" });
        let link = document.createElement("a");
        link.href = window.URL.createObjectURL(blob);
        link.download = "主检医生工作量统计.xls";
        link.click();
...
<style lang="less">
.main-doctor-workload-page-wrapper {
  width: 100%;
  min-width: 1300px;
  height: inherit;
