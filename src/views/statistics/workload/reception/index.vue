<template>
  <div class="reception-workload-page-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">前台登记员工作量统计</span>
      ...
      <div class="page-body clearfix">
        ...
        <div class="table-wrapper">
          ...
          <vxe-table class="mytable-footer"
            ...
            <vxe-table-column field="registerName"
                              title="登记员姓名"></vxe-table-column>
...
import { receptionWorkloadList, unitGroupInfo, receptionLeadout } from '@/api/workload'
import moment from 'moment'

export default {
  name: 'receptionWorkload',
  data () {
    return {
  ...
  methods: {
    ...
    getList () {
      ...
      this.isLoading = true
      receptionWorkloadList(this.form).then(({ data }) => {
    ...
    async leadoutExcelHandle () {
      ...
      try {
        const { data } = await receptionLeadout(params)
        this.leadoutLoading = false
        let blob = new Blob([data], { type: "application/vnd.ms-excel" });
        let link = document.createElement("a");
        link.href = window.URL.createObjectURL(blob);
        link.download = "前台工作量统计.xls";
        link.click();
...
<style lang="less">
.reception-workload-page-wrapper {
  width: 100%;
  min-width: 1300px;
  height: inherit;
