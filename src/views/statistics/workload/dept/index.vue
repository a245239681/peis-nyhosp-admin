<template>
  <div class="dept-workload-page-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">科室工作量统计</span>
      ...
      <div class="page-body clearfix">
        ...
        <div class="table-wrapper">
          <div class="search-info">符合查询条件记录共：<span>{{totalNum}}</span>条，实检人次<span>{{registerPeopleNum}}</span>次，
            实际体检<span>{{checkPeopleNum}}</span>人，工作量分析如下：</div>
          <vxe-table class="mytable-footer"
            ...
            <vxe-table-column field="checkComboPeopleNum"
                              title="实检人次"
                              width="200"></vxe-table-column>
            <vxe-table-column field="checkComboTotalPrice"
                              title="实检金额"
                              width="200">
              <template v-slot="{row}">
                <span>{{row.checkComboTotalPrice || row.checkComboTotalPrice == 0 ? Number(row.checkComboTotalPrice).toFixed(2) : ''}}</span>
              </template>
            </vxe-table-column>
          </vxe-table>
        </div>
...
import { deptWorkloadList, unitGroupInfo, deptLeadout } from '@/api/workload'
...
export default {
  ...
  data () {
    return {
      ...
      totalNum: 0, // 查询记录
      registerPeopleNum: 0, // 预登记人数
      checkPeopleNum: 0, // 已检人数
      leadoutLoading: false,
      totalPeopleNum: 0, // 合计字段：累计检查人数
      totalPrice: 0 //合计入参字段：累计金额
    }
  },
  computed: {
    // 体检科室列表
  ...
  methods: {
    ...
    getList () {
      ...
      this.isLoading = true
      deptWorkloadList(this.form).then(({ data }) => {
        this.isLoading = false
        if (data.code == 0) {
          let result = data.data
          this.tableData = this.getGroupSummary(result.deptList)
          this.totalNum = result.num
          this.registerPeopleNum = result.registerPeopleNum
          this.checkPeopleNum = result.checkPeopleNum
        } else {
          this.tableData = []
          this.totalNum = 0
          this.registerPeopleNum = 0
          this.checkPeopleNum = 0
    ...
    async leadoutExcelHandle () {
      ...
      params.departmentsResultList = this.tableData
      params.totalPeopleNum = this.totalPeopleNum
      params.totalPrice = this.totalPrice

      this.leadoutLoading = true
      try {
        const { data } = await deptLeadout(params)
        this.leadoutLoading = false
        let blob = new Blob([data], { type: "application/vnd.ms-excel" });
        let link = document.createElement("a");
        link.href = window.URL.createObjectURL(blob);
        link.download = "科室工作量统计.xls";
        link.click();
    ...
    getGroupSummary (data) {
      ...
      XEUtils.eachTree(data, (row) => {
        ...
        if (children && children.length) {
          ...
          children.push({
            deptName: `合计：`,
            checkComboPeopleNum: row.checkComboPeopleNum,
            checkComboTotalPrice: row.checkComboTotalPrice
          })
        }
      }, this.tableTreeConfig)
      return data
    ...
    handleSummary (children) {
      return {
        checkComboPeopleNum: XEUtils.sum(children, 'checkComboPeopleNum'),
        checkComboTotalPrice: (Math.round(XEUtils.sum(children, 'checkComboTotalPrice') * 100) / 100).toFixed(2)
      }
    },
      ...
      return [
        columns.map((column, columnIndex) => {
          ...
          switch (column.property) {
            case 'checkComboPeopleNum':
              this.totalPeopleNum = Math.round(XEUtils.sum(data, 'checkComboPeopleNum') * 100) / 100
              return `累计检查人次：\r${Math.round(XEUtils.sum(data, 'checkComboPeopleNum') * 100) / 100}`
            case 'checkComboTotalPrice':
              this.totalPrice = Math.round(XEUtils.sum(data, 'totalPrice') * 100) / 100
