<template>
  <div class="final-doctor-workload-page-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">总检医生工作量统计</span>
      ...
      <div class="page-body clearfix">
        ...
        <div class="table-wrapper">
          <div class="search-info">符合查询条件总检医生人数共：<span>{{tableData.length}}</span>人，工作量分析如下：</div>
...
import { finalDoctorWorkloadList, unitGroupInfo, finalDoctorLeadOut } from '@/api/workload'
import moment from 'moment'

export default {
  name: 'finalDoctorWorkload',
  data () {
    return {
  ...
  methods: {
    ...
    getList () {
      ...
      this.isLoading = true
      finalDoctorWorkloadList(this.form).then(({ data }) => {
    ...
    async leadoutExcelHandle () {
      ...
      params.doctorWorkloadResultList = this.tableData
      params.totalNumber = this.totalNumber

      this.leadoutLoading = true
      try {
        const { data } = await finalDoctorLeadOut(params)
        this.leadoutLoading = false
        let blob = new Blob([data], { type: "application/vnd.ms-excel" });
        let link = document.createElement("a");
        link.href = window.URL.createObjectURL(blob);
        link.download = "总检医生工作量统计.xls";
        link.click();
      ...
      return [
        columns.map((column, columnIndex) => {
          ...
          switch (column.property) {
            case 'peopleNumber':
              this.totalNumber = Math.round(XEUtils.sum(data, 'peopleNumber') * 100) / 100
              return `累计检查人数：${Math.round(XEUtils.sum(data, 'peopleNumber') * 100) / 100}`
          }
          return '-'
        })
      ]
...
<style lang="less">
.final-doctor-workload-page-wrapper {
  width: 100%;
  min-width: 1300px;
  height: inherit;
