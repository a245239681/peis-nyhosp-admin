<template>
  <div class="health-table-workload-page-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">体检表发放工作量统计</span>
...
import { healthTableWorkloadList, unitGroupInfo, healthTableLeadout } from '@/api/workload'
import moment from 'moment'

export default {
  name: 'healthTableWorkload',
  data () {
    return {
  ...
  methods: {
    ...
    getList () {
      ...
      this.isLoading = true
      healthTableWorkloadList(this.form).then(({ data }) => {
    ...
    async leadoutExcelHandle () {
      ...
      try {
        const { data } = await healthTableLeadout(params)
        this.leadoutLoading = false
        let blob = new Blob([data], { type: "application/vnd.ms-excel" });
        let link = document.createElement("a");
        link.href = window.URL.createObjectURL(blob);
        link.download = "体检表发放工作量统计.xls";
        link.click();
...
<style lang="less">
.health-table-workload-page-wrapper {
  width: 100%;
  min-width: 1300px;
  height: inherit;
