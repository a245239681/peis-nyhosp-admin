<template>
  <div class="guide-sheet-workload-page-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">指引单回收工作量统计</span>
...
import { guideSheetWorkloadList, unitGroupInfo, guideSheetLeadOut } from '@/api/workload'
import moment from 'moment'

export default {
  name: 'guideSheetWorkload',
  data () {
    return {
  ...
  methods: {
    ...
    getList () {
      ...
      this.isLoading = true
      guideSheetWorkloadList(this.form).then(({ data }) => {
    ...
    async leadoutExcelHandle () {
      ...
      try {
        const { data } = await guideSheetLeadOut(params)
        this.leadoutLoading = false
        let blob = new Blob([data], { type: "application/vnd.ms-excel" });
        let link = document.createElement("a");
        link.href = window.URL.createObjectURL(blob);
        link.download = "指引单回收工作量统计.xls";
        link.click();
      ...
      return [
        columns.map((column, columnIndex) => {
          ...
          switch (column.property) {
            case 'number':
              this.totalNumber = Math.round(XEUtils.sum(data, 'number') * 100) / 100
              return `累计回收指引单数：${Math.round(XEUtils.sum(data, 'number') * 100) / 100}`
          }
          return '-'
        })
      ]
...
<style lang="less">
.guide-sheet-workload-page-wrapper {
  width: 100%;
  min-width: 1300px;
  height: inherit;
