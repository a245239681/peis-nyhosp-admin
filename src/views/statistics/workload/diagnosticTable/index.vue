<template>
  <div class="diagnostic-table-workload-page-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">诊台医生工作量统计</span>
      ...
      <div class="page-body clearfix">
        ...
        <div class="table-wrapper">
          <div class="search-info">符合查询条件的体检项目组合共<span>{{dataCount}}</span>项，
            实检人次<span>{{peopleCount}}</span>次，工作量分析如下：</div>
          <vxe-table class="mytable-footer"
                     ...
                     :loading="isLoading">
            <vxe-table-column field="checkDoctorName"
                              title="医生姓名"
            ...
            <vxe-table-column field="comboPrice"
                              ...
                              width="120">
              <template v-slot="{row}">
                <span>{{row.comboPrice.toFixed(2)}}</span>
              </template>
            </vxe-table-column>
            <vxe-table-column field="peopleNumber"
                              title="实检人数"
                              width="120"></vxe-table-column>
            <vxe-table-column field="checkPrice"
                              title="实检金额"
                              width="140">
              <template v-slot="{row}">
                <span>{{row.checkPrice.toFixed(2)}}</span>
              </template>
            </vxe-table-column>
            <vxe-table-column field="countPrice"
                              title="合计金额"
                              width="140">
              <template v-slot="{row}">
                <span>{{row.countPrice.toFixed(2)}}</span>
              </template>
            </vxe-table-column>
          </vxe-table>
        </div>
...
import { diagnosticWorkloadList, unitGroupInfo, diagnosticLeadout } from '@/api/workload'
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      this.isLoading = true
      diagnosticWorkloadList(this.form).then(({ data }) => {
        this.isLoading = false
        if (data.code == 0) {
          let result = data.data
          this.tableData = result.checkDoctorWorkloadList
    ...
    async leadoutExcelHandle () {
      ...
      params.checkDoctorWorkloadList = this.tableData
      this.leadoutLoading = true
      try {
        const { data } = await diagnosticLeadout(params)
        this.leadoutLoading = false
        let blob = new Blob([data], { type: "application/vnd.ms-excel" });
        let link = document.createElement("a");
        link.href = window.URL.createObjectURL(blob);
        link.download = "诊台医生工作量统计.xls";
        link.click();
...
<style lang="less">
.diagnostic-table-workload-page-wrapper {
  width: 100%;
  min-width: 1300px;
  height: inherit;
