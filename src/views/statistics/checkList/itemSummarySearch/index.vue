<template>
  <div class="item-summary-search-page-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">项目组合小结统计</span>
          ...
          <div class="screen-form">
            <Form :model="form"
              ...
              <FormItem label="年龄范围"
                        :label-width="80"
                        prop="ageStart">
                <Input v-model.trim="form.ageStart"
                       style="width:60px"
                       size="small"
                       clearable />
              </FormItem>
              <FormItem label="到"
                        :label-width="36"
                        prop="ageEnd">
                <Input v-model.trim="form.ageEnd"
                       style="width:60px"
                       size="small"
                       clearable />
              </FormItem>
              <FormItem label="体检类型"
                        prop="categoryId"
                        :label-width="80">
                <Select v-model.trim="form.categoryId"
                        size="small"
              ...
              <FormItem label="组合名称"
                        ...
                        class="item-star"
                        prop="comboName">
                <Input v-model.trim="form.comboName"
                       style="width:140px"
                       size="small"
                       clearable />
              </FormItem>
              ...
              <FormItem label=""
                        :label-width="20"
                        prop="isUnit">
                <RadioGroup v-model.trim="form.isUnit"
              ...
              <FormItem label="体检单位"
                        prop="unitId"
                        :label-width="80">
                <Select v-model.trim="form.unitId"
                        class="unitSelect"
                        ref="unitSelectBox"
                        clearable
                        filterable
              ...
              <FormItem label="单位体检次数"
                        :label-width="100"
                        prop="unitVisitSn">
                <Select v-model.trim="form.unitVisitSn"
                        size="small"
              ...
              <FormItem label="体检分组"
                        prop="groupId"
                        :label-width="80">
                <Select v-model.trim="form.groupId"
                        size="small"
                        style="width:140px"
                        :disabled="unitInfoBtn"
...
import { itemSummaryList, unitGroupInfo, itemSummaryLeadout } from '@/api/workload'
import moment from 'moment'

export default {
  name: 'itemSummarySearch',
  data () {
    return {
  ...
  methods: {
    ...
    getList (page = 1, size = 20) {
      if (!this.form.comboName || this.form.comboName == '') {
        this.$Message.info('请输入项目组合名称后查询')
        return false
      }
      this.pageIndex = page
      this.pageSize = size
      ...
      itemSummaryList(pagination, this.form).then(({ data }) => {
    ...
    async leadoutExcelHandle () {
      ...
      try {
        const { data } = await itemSummaryLeadout(this.form)
        this.leadoutLoading = false
        let blob = new Blob([data], { type: 'application/vnd.ms-excel' })
        let link = document.createElement('a')
        link.href = window.URL.createObjectURL(blob)
        link.download = '项目组合小结统计.xlsx'
        link.click()
...
<style lang="less">
.item-summary-search-page-wrapper {
  width: 100%;
  min-width: 1300px;
  height: inherit;
