<template>
  <div class="sort-list-search-page-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">分检名单查询</span>
      </div>
        ...
        <div class="table-wrapper">
          ...
          <vxe-table class="mytable-footer"
            ...
            <vxe-table-column field="comboName"
                              ...
                              show-overflow="title"
                              min-width="140"></vxe-table-column>
            <vxe-table-column field="discountValue"
                              title="价格"
                              show-overflow="title"
...
import { unitGroupInfo, sortExamList, sortExamLeadout } from '@/api/workload'
...
export default {
  ...
  methods: {
    ...
    getList (page = 1, size = 20) {
      ...
      this.isLoading = true
      sortExamList(pagination, this.form).then(({ data }) => {
    ...
    async leadoutExcelHandle () {
      ...
      try {
        const { data } = await sortExamLeadout(this.form)
        this.leadoutLoading = false
        let blob = new Blob([data], { type: 'application/vnd.ms-excel' })
        let link = document.createElement('a')
        link.href = window.URL.createObjectURL(blob)
        link.download = '分检名单统计.xlsx'
        link.click()
