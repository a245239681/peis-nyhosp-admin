<template>
  <div class="check-list-search-page-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">体检名单查询</span>
          ...
          <div class="screen-form">
            <Form :model="form"
              ...
              <FormItem label="体检状态"
                        :label-width="80"
                        prop="resultStatuss">
                <Select v-model="form.resultStatuss"
                        multiple
                        size="small"
                        clearable
          ...
          <vxe-table class="mytable-footer"
            ...
            <vxe-table-column field="resultStatus"
              ...
              <template v-slot="{ row }">
                ...
                <span v-if="row.resultStatus == 0">未检</span>
                <span v-if="row.resultStatus == 1">在检</span>
                <span v-if="row.resultStatus == 3">待主检</span>
                <span v-if="row.resultStatus == 7">待总检</span>
                <span v-if="row.resultStatus == 9">已总检</span>
              </template>
            </vxe-table-column>
...
import { medicalWorkloadList, unitGroupInfo, medicalLeadout } from '@/api/workload'
...
export default {
  ...
  mounted () {
    ...
    this.$set(this.form, 'medicalStartDate', curDate)
    this.$set(this.form, 'medicalEndDate', curDate)
  ...
  methods: {
    ...
    getList (page = 1, size = 20) {
      console.log(this.form.resultStatuss)
      this.pageIndex = page
      this.pageSize = size
      ...
      // 格式化时间
      this.form.medicalStartDate = this.form.medicalStartDate != '' ? moment(this.form.medicalStartDate).format('YYYY-MM-DD') : ''
      this.form.medicalEndDate = this.form.medicalEndDate != '' ? moment(this.form.medicalEndDate).format('YYYY-MM-DD') : ''
      ...
      this.isLoading = true
      medicalWorkloadList(pagination, this.form).then(({ data }) => {
    ...
    // 更改人员来源
    personSourceChange (e) {
      if (e) {
        this.form.boSourceId = e.value
        this.form.boSourceName = e.label
      } else {
        this.form.boSourceId = ''
        this.form.boSourceName = ''
      }
    },
    ...
    async leadoutExcelHandle () {
      ...
      try {
        const { data } = await medicalLeadout(params)
...
<style lang="less">
.check-list-search-page-wrapper {
  width: 100%;
  min-width: 1300px;
  height: inherit;
