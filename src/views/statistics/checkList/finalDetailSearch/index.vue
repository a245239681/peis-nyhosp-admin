<template>
  <div class="final-detail-search-page-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">总检综述建议查询</span>
      </div>
      <div class="page-body clearfix">
        <div class="screen-info-wrapper">
          <div class="screen-form">
            <Form :model="form"
              ...
              <FormItem label="总检医生"
                        prop="finalReviewDoctorId"
                        :label-width="80">
                <Select v-model="form.finalReviewDoctorId"
                        :label-in-value="true"
                        filterable
                        clearable
        ...
        <div class="table-wrapper">
          ...
          <vxe-table class="mytable-footer"
                     ...
                     :height="(screenHeight - 450) <= 350 ? 350 : (screenHeight - 450)"
                     :data="tableData"
                     :loading="isLoading">
            ...
            <vxe-table-column field="groupingName"
                              ...
                              show-overflow="title"
                              min-width="120"></vxe-table-column>
            <vxe-table-column field="review"
                              title="综述"
                              show-overflow="tooltip"
                              min-width="160">
            </vxe-table-column>
            <vxe-table-column field="suggest"
                              title="建议"
                              show-overflow="tooltip"
                              min-width="160">
            </vxe-table-column>
            <vxe-table-column field="diseasePopularizationScience"
                              title="科普"
                              show-overflow="tooltip"
                              min-width="160">
            </vxe-table-column>
            <vxe-table-column field="finalReviewDoctor"
                              title="总检医生"
                              show-overflow="title"
                              min-width="120"></vxe-table-column>
            <vxe-table-column field="finalReviewDate"
                              title="总检时间"
                              width="160"
...
import { unitGroupInfo, finalDetialList, finalDetialLeadout } from '@/api/workload'
import moment from 'moment'

export default {
  name: 'finalDetailSearch',
  data () {
    return {
  ...
  mounted () {
    ...
    this.$store.dispatch('getExamDoctorDict', { params: { deptName: '总检医生', deptId: '' }, btn: true })
      .then(({ data }) => {
        this.mainExamDoctorList = data
      })
  ...
  methods: {
    ...
    getList (page = 1, size = 20) {
      ...
      this.isLoading = true
      finalDetialList(pagination, params).then(({ data }) => {
    ...
    mainExamDoctorChange (obj) {
      if (obj) {
        this.form.finalReviewDoctor = obj.label
        this.form.finalReviewDoctorId = obj.value
      } else {
        this.form.finalReviewDoctor = ''
        this.form.finalReviewDoctorId = ''
      }
    },
    ...
    deptChange (value, selectedData) {
      if (selectedData.length == 2) {
        // 即选择了父级部门，又选择了子级部门
        this.form.subDepartmentName = selectedData[1].deptName
        this.form.subDepartmentId = selectedData[1].deptId
        this.form.departmentId = selectedData[1].parentId
        this.form.departmentName = selectedData[1].parentName
        // console.log(this.formData)
      } else if (selectedData.length == 1) {
        // 只选择了父级
        this.form.subDepartmentName = ''
        this.form.subDepartmentId = ''
        this.form.departmentId = selectedData[0].deptId
        this.form.departmentName = selectedData[0].name
      } else {
        // 什么都没有选
        this.form.subDepartmentName = ''
        this.form.subDepartmentId = ''
    ...
    async leadoutExcelHandle () {
      ...
      try {
        const { data } = await finalDetialLeadout(params)
        this.leadoutLoading = false
        let blob = new Blob([data], { type: 'application/vnd.ms-excel' })
        let link = document.createElement('a')
        link.href = window.URL.createObjectURL(blob)
        link.download = '总检综述建议名单统计.xlsx'
        link.click()
    ...
    changeUnit (obj) {
      if (obj) {
        ...
        this.$set(this.form, 'unitVisitSn', '')
        this.$set(this.form, 'groupId', '')
        this.$set(this.form, 'departmentId', '')
        this.$set(this.form, 'subDepartmentId', '')
        this.$set(this.form, 'subDepartmentName', '')
        this.$set(this.form, 'departmentName', '')
        this.deptList = []
        this.selectDeptArr = []
        this.getGroups()
      }
...
<style lang="less">
.final-detail-search-page-wrapper {
  width: 100%;
  min-width: 1300px;
  height: inherit;
