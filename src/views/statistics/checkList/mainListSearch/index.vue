<template>
  <div class="main-list-search-page-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">主检名单查询</span>
      </div>
        ...
        <div class="screen-info-wrapper">
          <div class="screen-form">
            <Form :model="form"
              ...
              <FormItem label="检查时间"
                        :label-width="80"
                        prop="mainOperatorDateStartDate">
                <DatePicker type="date"
                            style="width:140px"
                            v-model="form.mainOperatorDateStartDate"
                            size="small"></DatePicker>
              </FormItem>
              <FormItem label="到"
                        :label-width="36"
                        prop="mainOperatorDateEdDate">
                <DatePicker type="date"
                            style="width:140px"
                            v-model="form.mainOperatorDateEdDate"
                            size="small"></DatePicker>
              </FormItem>
              <FormItem label=""
                        :label-width="20"
        ...
        <div class="table-wrapper">
          ...
          <vxe-table class="mytable-footer"
            ...
            <vxe-table-column field="doctorName"
                              ...
                              show-overflow="title"
                              min-width="120"></vxe-table-column>
            <vxe-table-column field="mainOperatorDate"
                              title="主检时间"
                              width="160"
...
import { unitGroupInfo, mainExamList, mainExamLeadout } from '@/api/workload'
...
export default {
  ...
  mounted () {
    ...
    this.$set(this.form, 'mainOperatorDateStartDate', curDate)
    this.$set(this.form, 'mainOperatorDateEdDate', curDate)
  },
  methods: {
    // 获取总检医生工作量统计列表
    getList (page = 1, size = 20) {
      ...
      // 格式化时间
      this.form.mainOperatorDateStartDate = this.form.mainOperatorDateStartDate != '' ? moment(this.form.mainOperatorDateStartDate).format('YYYY-MM-DD') : ''
      this.form.mainOperatorDateEdDate = this.form.mainOperatorDateEdDate != '' ? moment(this.form.mainOperatorDateEdDate).format('YYYY-MM-DD') : ''
      this.isLoading = true
      mainExamList(pagination, this.form).then(({ data }) => {
    ...
    async leadoutExcelHandle () {
      ...
      try {
        const { data } = await mainExamLeadout(this.form)
        this.leadoutLoading = false
        let blob = new Blob([data], { type: 'application/vnd.ms-excel' })
        let link = document.createElement('a')
        link.href = window.URL.createObjectURL(blob)
        link.download = '主检名单统计.xlsx'
        link.click()
