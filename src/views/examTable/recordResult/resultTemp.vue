<template>
  <div>
          ...
          <div class="flex-wrapper">
            ...
            <div style="width:25%; padding: 0 10px;">
              <div class="scan-code-input-wrapper">
                ...
                <FormItem label="">
                  <Input v-model.trim="registerCode"
                         clearable
                         ref="inputFocus"
    ...
    <div class="result-wrapper">
        ...
        <ul class="combo-list"
          ...
          <li class="combo-item"
            ...
            <div class="combo-handle"
              ...
              <Button type="error"
                      @click.stop="refuseExamHandle(item.boIsCompleted)"
      ...
      <div class="main">
        <Table :columns="(curShowType == '0' ? tableCols2 : tableCols)"
          ...
          <template slot-scope="{ row, index }"
            ...
            <Button size="small"
                    type="default"
                    @click="uploadShowHandle">上传</Button>
            <Button size="small"
          ...
          <div class="summary">
            <Tabs size="small"
              ...
              <TabPane label="疾病科普"
                       name="diseaseScience"
                       tab="summaryTab">
                <Input v-model="allDiseaseScience"
                       type="textarea"
                       :autosize="{minRows: 5,maxRows: 5}"
                       placeholder="请输入疾病科普" />
              </TabPane>
    ...
    <div class="record-dept">
      ...
      <div class="doctor">
        <span class="pre-title">检查者</span>
        <Select v-model="doctorId"
                filterable
                placement="top-end"
                transfer
                size="small">
          <template v-if="examDoctorDict.length > 0">
            <Option v-for="(item, index) in examDoctorDict"
                    :value="item.doctorId"
                    @click.native="examDoctorSelect(item)"
    ...
    <Modal v-model="uploadBtn"
      ...
      <Upload type="drag"
              ...
              :data="uploadData"
              :action="$url + `/booking/patientVisitPic/uploadFile`">
...
import {  getCombo, getSingleItem, resultContrast, patientVisitPic, refuseExam, delExamResult,
  resultSave, BMICaculate, WHRCaculate, recordSummary, PatientMsgByRegisterCode, checkReport, deleteReport, uploadPdf} from '@/api/record_result'
...
export default {
  ...
  data () {
    return {
      ...
      diseaseListVisible: false, // 检查结果列表弹窗
      comboList: [], // 项目组合列表
      branchExtraTip: '未见异常', // 分科小结（选填）
      branchTipList: [], // 分科小结列表
      branchAdvice: '', // 分科建议（细项）
      ...
      itemHip: '',
      WHRDiagnose: '',
      WHRBranch: '0', // 判断WHR是否进入小结
      WHRbranchAdvice: '', // WHR分科建议
      WHRDiseaseId: '', // WHR疾病id
      ...
      delExamLoading: false, // 删除结果请求loading
      curcomboBoId: '', // 当前选中组合项主键， 拒检接口需要获取
      curResultList: [], // 当前所选细项检查结果
      diseaseListTitle: 0, // 结果弹出框title，0为检查，1为检验
      allBranchAdvice: '',
      diseaseScience: '', // 疾病科普
      BMIdiseaseScience: '', // BMI疾病科普
      allDiseaseScience: '',
      uploadData: {}, // 上传参数
      importLoading: false,
      uploadFileName: '', // 已上传的非标文件名称
  ...
  watch: {
    ...
    branchAdvice (newVal) {
      this.allBranchAdvice = (this.branchAdvice + this.BMIbranchAdvice + this.WHRbranchAdvice)
    },
    BMIbranchAdvice (newVal) {
      this.allBranchAdvice = (this.branchAdvice + this.BMIbranchAdvice + this.WHRbranchAdvice)
    },
    WHRbranchAdvice (newVal) {
      this.allBranchAdvice = (this.branchAdvice + this.BMIbranchAdvice + this.WHRbranchAdvice)
    },
    diseaseScience (newVal) {
      this.allDiseaseScience = (this.diseaseScience + this.BMIdiseaseScience + this.WHRdiseaseScience)
    },
    BMIdiseaseScience (newVal) {
      this.allDiseaseScience = (this.diseaseScience + this.BMIdiseaseScience + this.WHRdiseaseScience)
    },
    WHRdiseaseScience (newVal) {
      this.allDiseaseScience = (this.diseaseScience + this.BMIdiseaseScience + this.WHRdiseaseScience)
    }
  },
  ...
  methods: {
    getExamResult (row, index) {
      ...
      if (row.paItemName.indexOf('腰围') >= 0) {
        this.waist = row.paRecord
        this.itemWaist = row.paItemName
        this.caculateWHR()
      }
      if (row.paItemName.indexOf('臀围') >= 0) {
        this.hip = row.paRecord
        this.itemHip = row.paItemName
        this.caculateWHR()
      }
      ...
      console.log(this.tableData[this.curComboId][index])
      let curId = row.paId
      let params
      if (row.paRecordType == '1') {
        ...
        if (row.paItemName == '血压' || this.curResultList.length == 0) {
          let result = this.tableData[this.curComboId][index].paRecord.trim()
      ...
      resultContrast(params).then(({ data }) => {
        ...
        if (data.code == 0) {
          ...
          // 建议或科普为空时，不做覆盖建议处理
          let _arr = result.resultList.filter(item => {
            return item.paDictSuggestionList.length > 0 || item.knowledgeList.length > 0
          })
          if (_arr.length > 0) {
            // 保存项目检查结果的疾病建议
            let adviceItem = {
    ...
    handleReset () {
      ...
      this.diseaseScience = ''
      this.allDiseaseScience = ''
      this.BMIdiseaseScience = ''
      this.WHRDiagnose = ''
      this.WHRbranchAdvice = ''
      this.WHRDiseaseId = ''
      this.WHRdiseaseScience = ''
      this.uploadFileName = ''
      // 重置table input框聚焦
    ...
    async getComboItem (item) {
      ...
      this.uploadData.comboId = item.boComboCode // 保存上传非标参数
      this.uploadFileName = ''
      ...
      await this.$store.dispatch('getExamDoctorDict', { params: obj, btn: true }).then(({ data }) => {
        ...
        let isHasDoctor = data.some(ele => {
          return this.userInfo.userId == ele.doctorId
        })
        if (isHasDoctor) {
      ...
      // 获取小结、建议、病史模板信息
      // this.$refs.demoInput.getConclusion(item.boDeptId)
      // this.$refs.demoInput.getMedicalHistory(item.boDeptId)
      // this.$refs.demoInput.getSuggest(item.boDeptId)
    ...
    popupShowHandle () {
      this.popupBtn = true
      // this.$refs.demoInput.getCurrentDemo(this.curTabIndex)
    ...
    getPatientInfo (row) {
      ...
      this.uploadData.peId = row.boPeId
      this.uploadData.visitSn = row.boVisitSn
      ...
      if (this.height && this.itemHeight && this.weight && this.itemWeight) {
        ...
        BMICaculate(params).then(({ data }) => {
          if (data.code == 0) {
            ...
            let BMIScience = data.data[0].dictSuggestion.knowledgeList
            let cont = ''
            let cont2 = ''
            let _diseaseName = data.data[0].dictSuggestion.paDiseaseName
            ...
            if (BMIScience && BMIScience.length > 0) {
              BMIScience.forEach(ele => {
                cont2 += ele + '\n'
              })
            }
            ...
            if (cont) {
              if (ORGINFO.name && ORGINFO.name.indexOf('民主路') > -1) {
                this.BMIbranchAdvice = _diseaseName + '：\n' + cont
              } else {
                this.BMIbranchAdvice = _diseaseName + '：' + cont
              }
            } else {
            ...
            if (cont2) {
              if (ORGINFO.name && ORGINFO.name.indexOf('民主路') > -1) {
                this.BMIdiseaseScience = _diseaseName + '：\n' + cont2
              } else {
                this.BMIdiseaseScience = _diseaseName + '：' + cont2
              }
            } else {
              this.BMIdiseaseScience = cont2;
            }
            // cont && (this.BMIbranchAdvice = _diseaseName + '：' + cont)
            // cont2 && (this.BMIdiseaseScience = _diseaseName + '：' + cont2)
            this.getDiagnosisList()
          } else {
            this.$Message.error(data.message)
          }
    ...
    // 腰围臀围计算WHR值
    caculateWHR () {
      if (this.waist && this.itemWaist && this.hip && this.itemHip) {
        let params = {
          waist: this.waist,
          hip: this.hip,
          itemWaist: this.itemWaist,
          itemHip: this.itemHip,
          sex: this.form.boSex
        }
        WHRCaculate(params).then(({ data }) => {
          if (data.code == 0) {
            this.WHRDiagnose = data.data[0].paResultDescription
            this.WHRBranch = data.data[0].paItemSummary
            let WHRAdvice = data.data[0].dictSuggestion.paDictSuggestionList
            let WHRScience = data.data[0].dictSuggestion.knowledgeList
            let cont = ''
            let cont2 = ''
            let _diseaseName = data.data[0].dictSuggestion.paDiseaseName

            if (WHRAdvice.length > 0) {
              WHRAdvice.forEach(ele => {
                cont += ele + '\n'
              })
            }
            if (WHRScience && WHRScience.length > 0) {
              WHRScience.forEach(ele => {
                cont2 += ele + '\n'
              })
            }
            this.WHRDiseaseId = data.data[0].paDiseaseDiagnosisId

            if (cont) {
              if (ORGINFO.name && ORGINFO.name.indexOf('民主路') > -1) {
                this.WHRbranchAdvice = _diseaseName + '：\n' + cont
              } else {
                this.WHRbranchAdvice = _diseaseName + '：' + cont
              }
            } else {
              this.WHRbranchAdvice = cont
            }
            if (cont2) {
              if (ORGINFO.name && ORGINFO.name.indexOf('民主路') > -1) {
                this.WHRdiseaseScience = _diseaseName + '：\n' + cont2
              } else {
                this.WHRdiseaseScience = _diseaseName + '：' + cont2
              }
            } else {
              this.WHRdiseaseScience = cont2;
            }
            this.getDiagnosisList()
          } else {
            this.$Message.error(data.message)
    ...
    getReport () {
      ...
      checkReport(params).then(({ data }) => {
        ...
        if (data.code == 0) {
          ...
          this.$Message.warning('该项目未上传报告，请上传保存后在查看')
        }
      }).catch(err => {
        this.reportLoading = false
    ...
    getItemAdvice () {
      ...
      // for (let i in this.branchAdviceList) {
      //   let item = this.branchAdviceList[i]

      //   let arr = item.resultList
      //   console.log(arr)
      //   arr.forEach(ele => {
      //     let diseaseName = ele.paDiseaseName
      //     let knowledgeArr = ele.knowledgeList
    ...
    // 获取病人套餐
    // getPatientPackage (id, visitSn) {
    //   let params = {
    //     peId: id,
    //     visitSn: visitSn
    //   }
    //   getSelectPackage(params).then(({ data }) => {
    //     if (data.code == 0) {
    //       let result = data.data
    //       if (result) {
    //         this.$set(this.form, 'boPackage', result.boPackage)
    //         this.$set(this.form, 'boPackageId', result.boPackageId)
    //       }
    //     }
    //   })
    // },
    ...
    getPatientByRegisterCode () {
      ...
      PatientMsgByRegisterCode(params).then(({ data }) => {
        if (data.code == 0) {
          ...
          this.uploadData.peId = result[0].boPeId
          this.uploadData.visitSn = result[0].boVisitSn
          if (result[1].pic != '') {
    ...
    getDiagnosisList () {
      ...
      console.log(this.curComboId)
      this.tableData[this.curComboId].forEach((item, index, arr) => {
        let itemData = ''
        if (item.paIsAbnormal && item.paRecord != '' || item.isFreeHand && item.paRecord != '' || this.curShowType === '0' && item.paRecord != '') {
        ...
        // if (item.paItemSummary && item.paItemSummary == '0') {
        //   branchtipData += item.paResultDescription + '\n'
        // }
        ...
        if (item.paItemName.indexOf('腰围') >= 0 && this.WHRDiagnose) {
          this.diagnosisList += '腰臀比：' + this.WHRDiagnose + '\n'
          if (this.WHRBranch == '0') {
            branchtipData += '腰臀比' + this.WHRDiagnose + '  '
          }
        }
      });
    ...
    save () {
      ...
      let params = {
        ...
        showType: '1', // 区分检验检查 1为检查 3为检验
        bmiDiseaseId: this.BMIDiseaseId,
        diseasePopularizationScience: this.allDiseaseScience,
        imgName: this.uploadFileName || '',
        registerCode: this.registerCode || ''
      }
    ...
    bindResultInput (row, index) {
      ...
      if (row.paRecord.trim() == '' || row.paRecord.trim() == '/') {
        ...
        if (row.paItemName.indexOf('腰围') >= 0 || row.paItemName.indexOf('臀围') >= 0) {
          this.WHRDiagnose = ''
        }
        this.getDiagnosisList()
      }
    ...
    handleUpload (file) {
      ...
      let config = {
        peId: this.uploadData.peId,
        comboId: this.uploadData.comboId,
        visitSn: this.uploadData.visitSn
      };
      this.importLoading = true
      uploadPdf(config, formData).then(({ data }) => {
        this.importLoading = false
        if (data.code == 0) {
          this.uploadFileName = data.data
          this.uploadBtn = false
          this.$Message.success('上传成功，需要保存才能查看上传的报告')
        } else {
          this.$Message.error(data.message)
        }
      }).catch(err => {
