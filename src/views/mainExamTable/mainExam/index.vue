  ...
  <div class="main-exam-wrapper"
            ...
            <div class="content-container">
              <div class="textarea">
                <Input v-model="newReview"
                       type="textarea"
                       :rows="11"
                       :disabled="formData.boResultStatus == 9 || reviewModalOpen"
            ...
            <div class="content-container">
              <div class="textarea">
                <Input v-model="newSuggest"
                       type="textarea"
                       :rows="11"
                       :disabled="formData.boResultStatus == 9 || suggestModalOpen"
          ...
          <div class="btn-box">
            <Row :gutter="16">
              ...
              <div class="left-btn">
                ...
                <Button type="primary"
                        v-permission="['print']"
                        :disabled="!(formData.boResultStatus == 9)"
                        @click="applyPrint"
                        size="small"
              ...
              <div class="right-btn">
                保存自动清空：
                <i-switch v-model="isMainExamAutoClear"
                          style="margin-right:10px;"
                ...
                <Button type="primary"
                        v-if="formData.boResultStatus == 9"
                        size="small"
                        v-permission="['edit']"
                        hidefocus="true"
                        @click="cancelMainExam">取消总检</Button>
                <Button type="primary"
                        v-else
                        :loading="loading"
                        size="small"
                        v-permission="['edit']"
                        hidefocus="true"
                        @click="saveMainExam">保存总检</Button>
    ...
    <div class="main-exam-footer-wrapper">
      <div class="footer-info">
        <div class="left-side">
          <span class="pre-title">总检结论</span>
        ...
        <div class="right-side">
          <div class="footer-item">
            ...
            <Input v-model="mainExamDoctorName"
                   size="small"
                   style="width: 120px"
                   v-if="formData.boResultStatus == 9 || formData.boResultStatus == 7"
                   :disabled="formData.boResultStatus == 9 || formData.boResultStatus == 7" />
            <Select v-else
          ...
          <div class="footer-item">
            ...
            <DatePicker v-model="mainExamDate"
                        :value="mainExamDate"
                        placement="top-start"
                        type="datetime"
          ...
          <div class="footer-item">
            <span class="pre-title">总检审核医生</span>
            <Input v-model="doctorName"
                   size="small"
                   style="width: 120px"
                   disabled />
          </div>
        ...
        <div class="footer-item">
          ...
          <div class="footer-item">
            <span class="pre-title">总检审核日期</span>
...
export default {
  ...
  data () {
    return {
      ...
      isMainExamAutoClear: false, // 保存是否清空
  ...
  mounted () {
    ...
    if (!isConfig.isMainExamAutoClear) {
      setAutoClearStorage({ isMainExamAutoClear: this.isMainExamAutoClear })
    } else {
      this.isMainExamAutoClear = isConfig.isMainExamAutoClear
    }
  },
  destroyed () {
  ...
  methods: {
    ...
    checkMainExamRecord (val) {
      ...
      deptResultDetailsList(params).then(({ data }) => {
        ...
        if (data.code == 0) {
          ...
          if (otherInfo.isReview == '0' || val.boResultStatus == '9') {
            this.getInfo(val, '')
          } else {
            this.$Modal.confirm({
              title: '提示',
        ...
        .then(({ data }) => {
          ...
          if (data.code == 0) {
            ...
            this.appendResultId = otherInfo.appendResultId
              ...
              : ''
            this.operator = otherInfo.finalAuditName || this.userInfo.poName
            this.examDate = otherInfo.checkDate ? moment(otherInfo.checkDate).format('YYYY-MM-DD HH:mm') : moment().format('YYYY-MM-DD HH:mm')
            this.mainExamDate = otherInfo.mainAuditDate ? moment(otherInfo.mainAuditDate).format('YYYY-MM-DD HH:mm') : moment().format('YYYY-MM-DD HH:mm')
            // 总检结论
    ...
    async saveMainExam () {
      ...
      if (!this.doctorId) {
        this.$Message.warning('请选择总检医生，再进行保存总检操作')
        return
      }
      if (!this.mainExamDoctorId) {
        this.$Message.warning('请选择主检医生，再进行保存总检操作')
        return
      }
      if (!this.formData.boPeId && !this.formData.boVisitSn) {
        this.$Message.warning('信息不全，不能保存总检')
        return
      }
      ...
      let params = {
        ...
        finalDate: moment(this.examDate).format('YYYY-MM-DD HH:mm:ss'),
        mainAuditDate: moment(this.mainExamDate).format('YYYY-MM-DD HH:mm:ss'),
      ...
      if (this.uncheckedItems.length > 0 && (!setting.hasOwnProperty('mainExamTipFlag') || setting.mainExamTipFlag)) {
        this.$Modal.confirm({
          title: '提示',
          content: '还有未体检项目，是否继续保存总检',
          onOk: async () => {
            try {
              // 发送请求
              this.loading = true
              this.spinShow = true
              let type = 2 // type: 1为主检，2为总检
              ...
              if (data.code == 0) {
                ...
                this.formData.boResultStatus = '9'
                if (this.isMainExamAutoClear) {
                  this.handleReset()
                } else {
                  let param = {
    ...
    cancelMainExam () {
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          cancelMainExam(params)
            .then(({ data }) => {
              if (data.code == 0) {
                this.$Message.success('操作成功')
                this.formData.boResultStatus = '7'
                this.$refs.search.handleSearch()
              } else {
                this.$Message.error(data.message)
              }
    ...
    handleDbClickReview () {
      ...
      //   return false
      // }
      this.revModalInputStatus = this.formData.boResultStatus == 9
    ...
    handleDbClickSuggest () {
      ...
      // }
      // console.log(this.formData)
      this.sugModalInputStatus = this.formData.boResultStatus == 9
