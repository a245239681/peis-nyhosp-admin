<template>
  <div class="fee-day-balance-wrapper">
    <div class="fee-day-closure-wrapper">
      <Card style="width:100%;">
        <div slot="title"
             class="search-title-wrapper">
          <span class="title">当面付待日结-统计</span>
    ...
    <div class="statements-wrapper">
      <Card style="width:100%;height:100%;">
        <div slot="title"
             class="search-title-wrapper">
          <span class="title">当面付待日结-结算单明细</span>
          <Input search
                 enter-button
                 style="width: 200px"
                 size="small"
...
import {
  dailyDetailList,
  dailyBalance
} from '@/api/balance_feeDayBalance.js'
      ...
      dailyDetailList(pagination, params)
        .then(({ data }) => {
          ...
          if (data.code == 0) {
            ...
            let _col1 = { name: '收款金额（元）', total: totalFee, wx: wxPayTotal, zfb: zfbPayTotal, cash: realCash, union: unionPayTotal }
            let _col2 = { name: '费用笔数（笔）', total: totalCount, wx: wxPayTotalCount, zfb: zfbPayTotalCount, cash: realCashCount, union: unionPayTotalCount }
            let arr = []
            arr.push(_col1)
            arr.push(_col2)
