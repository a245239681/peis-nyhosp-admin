      ...
      <div class="modal-body">
        <div class="bill-info">
          ...
          <Form :model="formData"
            ...
            <FormItem label="发票号：">
              <Input v-model="formData.invoiceNo"></Input>
            </FormItem>
          </Form>
        </div>
        <div class="bill-preview">
          ...
          <div class="bill-number">
            <template v-if="formData.invoiceNo"><span>No.</span>{{formData.invoiceNo}}</template>
          </div>
          <div class="patient-name">姓名：{{billInfo.patientName}}</div>
          ...
          <div class="footer-tip">
            <div class="doctor-name">收款员：{{billInfo.drawerName}}</div>
            <div class="bill-date">收款日期：{{billInfo.payTime}}</div>
          </div>
        </div>
      </div>
      <div slot="footer">
...
<script>
import { billPreViewInfo, printBill } from '@/api/weChatBillManage'
...
export default {
  ...
  methods: {
    ...
    getInfo (obj) {
      this.isLoading = true
      billPreViewInfo(obj).then(({ data }) => {
        this.isLoading = false
        if (data.code == 0) {
          this.billInfo = data.data
          this.tableData = data.data.subjectList
          this.$set(this.formData, 'totalAmount', data.data.totalAmount)
        } else {
    ...
    // 打印发票
    printConfirmHandle () {
      const { patientName, invoiceNo, orderId, drawerName, registerCode } = this.billInfo
      const params = { patientName, invoiceNo, orderId, drawerName, registerCode }
      params.invoiceNo = this.formData.invoiceNo
      if (!printNotice()) return false
      this.billLoading = true
      printBill(params).then(({ data }) => {
        this.billLoading = false
        if (data.code == 0) {
          console.log(data)
          let printParams = {
            invoiceUrl: this.$url + data.data.invoicePdf,
        ...
        columns.map((column, columnIndex) => {
          ...
          switch (column.property) {
            case 'subjectAmount':
              ...
              return this.billInfo.totalAmount
          }
          return '-'
        })
      ]
