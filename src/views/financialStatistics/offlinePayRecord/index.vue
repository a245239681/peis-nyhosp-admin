<template>
  <div class="offline-pay-record-wrapper">
...
import { offlinePayRecord, payRecordLeadout } from '@/api/offlinePayRecord'
import moment from 'moment'

export default {
  name: 'offlinePayRecord',
  data () {
    return {
  ...
  mounted () {
    ...
    this.$set(this.form, 'chargeDateStart', curDate)
    this.$set(this.form, 'chargeDateEnd', curDate)
    this.getList()
  },
  methods: {
    // 获取支付记录列表
    getList (page = 1, size = 20) {
      ...
      // 格式化时间
      let _chargeDateStart = this.form.chargeDateStart != '' ? moment(this.form.chargeDateStart).format('YYYY-MM-DD') : ''
      let _chargeDateEnd = this.form.chargeDateEnd != '' ? moment(this.form.chargeDateEnd).format('YYYY-MM-DD') : ''
      const params = Object.assign({}, this.form)
      params.chargeDateStart = _chargeDateStart
      params.chargeDateEnd = _chargeDateEnd
      this.isLoading = true
      offlinePayRecord(pagination, params).then(({ data }) => {
