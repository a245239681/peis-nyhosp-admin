<template>
  <div class="branch-advice-wrapper">
    <Card class="branch-advice-left">
    ...
    <Card class="branch-advice-right">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">常用分科建议</span>
...
<script>
import { dictDeptSuggestList, dictDeptSuggestDelete, dictDeptSuggestStatus } from '@/api/busdict_branchAdvice.js'
...
export default {
  ...
  methods: {
    getList (page = 1, size = 20) {
      ...
      this.isLoading = true
      dictDeptSuggestList(pagination, params).then(({ data }) => {
        console.log(data)
    ...
    statusChange (row) {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          dictDeptSuggestStatus(params).then(({ data }) => {
            if (data.code == 0) {
    ...
    deleteData (row) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          dictDeptSuggestDelete(params).then(({ data }) => {
            if (data.code == 0) {
              this.$Message.success(`已删除该科室建议`)
              // 更新分科建议vuex状态
              let params = {
                deptId: this.userInfo.userDeptId
              }
    ...
    getComboList (item, index) {
      ...
      this.isLoading = true
      dictDeptSuggestList(pagination, params)
        .then(({ data }) => {
          console.log(data)
          this.isLoading = false
          if (data.code == 0) {
