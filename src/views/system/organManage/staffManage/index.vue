<template>
  <div class="staff-manage-wrapper">
    <Card class="staff-manage-left"
          shadow>
    ...
    <Card class="staff-manage-right"
          shadow>
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">工作人员维护</span>
...
<script>
import {
  staffMaintenanceList,
  staffMaintenanceDelete,
  staffMaintenanceStatus
} from '@/api/sys_staffManage.js'
...
export default {
  ...
  computed: {
    ...
    roomDict: {
      get: function () {
        return this.$store.getters.getFuncRoomDict
      },
      set: function () { }
    },
    roomFilter () {
  ...
  mounted () {
    this.$store.dispatch('getFuncRoomDict')
    this.getList()
    this.$nextTick(() => {
      this.curActive = 'all'
    })
  ...
  methods: {
    getList (page = 1, size = 20) {
      ...
      let params = {
        deptId: this.deptId,
        text: this.keywords.trim()
      }
      if (this.isLoading) {
        return false
      }
      this.isLoading = true
      staffMaintenanceList(pagination, params)
        .then(({ data }) => {
          console.log(data)
