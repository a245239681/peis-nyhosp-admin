...
<script>
import { roleList, roleDelete, roleStatus } from '@/api/roleManage.js'
...
export default {
  ...
  methods: {
    getList (page = 1, size = 20) {
      ...
      this.isLoading = true
      roleList(pagination, params).then(({ data }) => {
    ...
    statusChange (row) {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          roleStatus(params).then(({ data }) => {
            if (data.code == 0) {
              this.$Message.success(`已${msg}该角色`)
              this.getList(this.pageIndex, this.pageSize)
            } else {
              this.$Message.error(data.message)
            }
