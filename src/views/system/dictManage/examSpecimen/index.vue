<template>
  <div class="exam-specimen-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">体检标本管理</span>
...
<script>
import { specimanList, specimanDelete, dictSpecimanStatus } from '@/api/dict_examSpeciman.js'
...
export default {
  ...
  methods: {
    getList (page = 1, size = 20) {
      ...
      this.isLoading = true
      specimanList(pagination, params).then(({ data }) => {
    ...
    deleteData (row) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          specimanDelete(params).then(({ data }) => {
            if (data.code == 0) {
    ...
    statusChange (row) {
      ...
      this.$Modal.confirm({
        title: '提示',
        content: `是否${msg}该体检标本？`,
        onOk: () => {
          let params = {
            status: status,
            id: row.paId
          }
          dictSpecimanStatus(params).then(({ data }) => {
            if (data.code == 0) {
              this.$Message.success(`已${msg}该体检标本`)
              this.$store.dispatch('getSpecimanDict', true)
              this.getList(this.pageIndex, this.pageSize)
            } else {
              this.$Message.error(data.message)
