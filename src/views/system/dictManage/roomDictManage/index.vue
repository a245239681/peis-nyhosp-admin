<template>
  <div class="room-dictmanage-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">体检科室字典管理</span>
        <div class="title-right-tool">
          ...
          <Button type="primary"
                  ...
                  v-permission="['add']"
                  :loading="loading"
                  @click="importHis">导入his科室</Button>
          <!-- <Button type="error"
                  style="margin-left: 8px;"
...
import { dictDeptList, dictDeptSave, dictDeptUpdateStatus, dictDeptDelete, departmentDataSynchronous } from '@/api/dict_roomDictManage.js'
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        {
          title: '是否打印清单',
          slot: 'poIsPrtDeptRequest',
          key: 'poIsPrtDeptRequest',
          minWidth: 120,
          align: 'center'
        },
        {
  ...
  methods: {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          dictDeptUpdateStatus(params).then(({ data }) => {
            if (data.code == 0) {
              this.$Message.success(`已${msg}该体检科室`)
              this.$store.dispatch('getRoomDict', true)
              this.getList(this.pageIndex, this.pageSize)
            } else {
              this.$Message.error(data.message)
    ...
    importHis () {
      console.log('导入')
      // http://117.141.4.40:28087/api/his/departmentDataSynchronous
      let params = {
        DepartmentType: '',
        DepartmentClass: '',
        OrganizationId: ''
      ...
      this.loading = true
      departmentDataSynchronous(params).then(({ data }) => {
        this.loading = false
        console.log(data)
        if (data.code == 0) {
