<template>
  <div class="guide-sheet-group-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">指引单分组管理</span>
        <div class="title-right-tool">
          ...
          <Button type="primary"
                  ...
                  @click="add"
                  v-permission="['add']"
                  size="default">添加</Button>
          <!-- <Button type="error"
                  style="margin-left: 8px;"
                  @click="deleteAllHandle"
                  :disabled="selections.length == 0"
                  size="default">批量删除</Button> -->
        </div>
      </div>
...
<script>
import { guideTypeList, guideTypeDelete } from '@/api/dict_guideType.js'

import Edit from './edit.vue'

export default {
  name: 'guideSheetGroup',
  components: {
    Edit
  },
  data () {
  ...
  methods: {
    getList (page = 1, size = 20) {
      ...
      this.isLoading = true
      guideTypeList(pagination, params).then(({ data }) => {
    ...
    deleteData (row) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          guideTypeDelete(params).then(({ data }) => {
            if (data.code == 0) {
