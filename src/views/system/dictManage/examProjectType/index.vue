<template>
  <div class="exam-project-type-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">体检项目组合分类管理</span>
...
<script>
import { comboTypeList, comboTypeDelete, comboTypeStatus } from '@/api/dict_examProjectType.js'
import Edit from './edit.vue'

export default {
  name: 'examProjectType',
  components: {
    Edit
  },
  data () {
  ...
  methods: {
    getList (page = 1, size = 20) {
      ...
      this.isLoading = true
      comboTypeList(pagination, params).then(({ data }) => {
    ...
    deleteData (row) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          comboTypeDelete(params).then(({ data }) => {
            if (data.code == 0) {
    ...
    statusChange (row) {
      ...
      this.$Modal.confirm({
        title: '提示',
        content: `是否${msg}该体检项目组合分类？`,
        onOk: () => {
          let params = {
            status: status,
            id: row.paId
          }
          comboTypeStatus(params).then(({ data }) => {
            if (data.code == 0) {
              this.$Message.success(`已${msg}该体检项目组合分类`)
              this.$store.dispatch('getComboTypeDict', true)
              this.getList(this.pageIndex, this.pageSize)
            } else {
              this.$Message.error(data.message)
