<template>
  <div class="expense-type-dict-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">费用类别字典管理</span>
...
<script>
import {
  peDictReceiptTypeList,
  peDictReceiptTypeDelete,
  peDictReceiptTypeStatus
} from '@/api/dict_expenseTypeDict.js'
...
export default {
  ...
  methods: {
    getList (page = 1, size = 20) {
      ...
      this.isLoading = true
      peDictReceiptTypeList(pagination, params)
        .then(({ data }) => {
          console.log(data)
    ...
    statusChange (row) {
      ...
      this.$Modal.confirm({
        title: '提示',
        content: `是否${msg}该费用类别？`,
        onOk: () => {
          let params = {
            paStatus: status,
            paId: row.paId
          }
          peDictReceiptTypeStatus(params)
            .then(({ data }) => {
              if (data.code == 0) {
                this.$Message.success(`已${msg}该费用类别`)
    ...
    deleteData (row) {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          peDictReceiptTypeDelete(params)
            .then(({ data }) => {
              if (data.code == 0) {
                this.$Message.success(`已删除该费用类别数据`)
