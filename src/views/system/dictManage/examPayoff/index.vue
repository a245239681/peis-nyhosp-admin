...
<script>
import { dictChargeList, dictChargeSave, dictChargeDelete, dictChargeStatus } from '@/api/dict_examPayoff.js'
...
export default {
  ...
  methods: {
    getList (page = 1, size = 20) {
      ...
      this.isLoading = true
      dictChargeList(pagination, params).then(({ data }) => {
    ...
    statusChange (row) {
      ...
      this.$Modal.confirm({
        title: '提示',
        content: `是否${msg}该体检费别？`,
        onOk: () => {
          let params = {
            cdStatus: status,
            cdId: row.cdId
          }
          dictChargeStatus(params).then(({ data }) => {
            if (data.code == 0) {
              this.$Message.success(`已${msg}该体检费别`)
    ...
    deleteData (row) {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          dictChargeDelete(params).then(({ data }) => {
            if (data.code == 0) {
              this.$Message.success(`已删除该体检费别数据`)
              this.$store.dispatch('getChargeTypeDict', true)
