<template>
  <div class="medical-insurance-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">医保字典管理</span>
...
<script>
import { medicalList, medicalSave, medicalStatus, medicalDelete } from '@/api/medicalInsurance.js'
...
export default {
  ...
  methods: {
    getList (page = 1, size = 20) {
      ...
      this.isLoading = true
      medicalList(pagination, params).then(({ data }) => {
    ...
    statusChange (row) {
      ...
      this.$Modal.confirm({
        title: '提示',
        content: `是否${msg}该医保类型？`,
        onOk: () => {
          let params = {
            paStatus: status,
            paId: row.paId
          }
          medicalStatus(params).then(({ data }) => {
            if (data.code == 0) {
              this.$Message.success(`已${msg}该医保类型`)
    ...
    deleteData (row) {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          medicalDelete(params).then(({ data }) => {
            if (data.code == 0) {
              this.$Message.success(`已删除该医保类型数据`)
