<template>
  <div>
    <Modal v-model="isVisible"
           ...
           :title="showTitle"
           class="clinicalType-edit-wrapper">
      <div class="modal-body">
        <Form :model="formData"
          ...
          <FormItem label="临床类型名称"
                    prop="paClinicalTypeName"
                    :rules="[{ required: true, message: '请输入临床类型名称', trigger: 'blur' }]">
            <Input v-model="formData.paClinicalTypeName"
  ...
  methods: {
    ...
    save () {
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.isLoading = true
          clinicalSave(this.formData).then(({ data }) => {
    ...
    // 根据输入内容获取拼音码、五笔码
    searchCode () {
      let str = this.formData.paClinicalTypeName
      if (!str || str && str.trim() == '') {
        return false
      }
      let params = {
