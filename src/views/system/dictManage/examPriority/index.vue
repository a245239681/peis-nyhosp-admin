<template>
  <div class="exam-priority-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">体检优先级管理</span>
...
<script>
import { dictPriorityList, dictPriorityStatus, dictPriorityDelete } from '@/api/dict_examPriority.js'
...
export default {
  ...
  methods: {
    getList (page = 1, size = 20) {
      ...
      this.isLoading = true
      dictPriorityList(pagination, params).then(({ data }) => {
    ...
    statusChange (row) {
      ...
      this.$Modal.confirm({
        title: '提示',
        content: `是否${msg}该体检优先级？`,
        onOk: () => {
          let params = {
            cdStatus: status,
            cdId: row.cdId
          }
          dictPriorityStatus(params).then(({ data }) => {
            if (data.code == 0) {
              this.$Message.success(`已${msg}该体检优先级`)
    ...
    deleteData (row) {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          dictPriorityDelete(params).then(({ data }) => {
            if (data.code == 0) {
              this.$Message.success(`已删除该体检优先级数据`)
              this.$store.dispatch('getPriorityDict', true)
