<template>
  <div>
    <Modal v-model="isVisible"
           ...
           :title="showTitle"
           class="professionDict-edit-wrapper">
      <div class="modal-body">
        <Form :model="formData"
          ...
          <FormItem label="职业名称"
                    prop="sdName"
                    :rules="[{ required: true, message: '请输入职业名称', trigger: 'blur' }]">
          ...
          <FormItem label="职业编号"
                    prop="sdCode"
                    :rules="[{ required: true, message: '请输入职业编号', trigger: 'blur' }]">
...
<script>
import { occupationSave } from '@/api/dict_profession.js'
  ...
  methods: {
    ...
    save () {
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.isLoading = true
          occupationSave(this.formData).then(({ data }) => {
