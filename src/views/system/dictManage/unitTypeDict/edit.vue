<template>
  <div>
    <Modal v-model="isVisible"
           ...
           :title="showTitle"
           class="unitTypeDict-edit-wrapper">
      <div class="modal-body">
        <Form :model="formData"
          ...
          <FormItem label="单位类别名称"
                    prop="sdName"
                    :rules="[{ required: true, message: '请输入单位类别名称', trigger: 'blur' }]">
            <Input v-model="formData.sdName"
                   @on-change="searchCode"
                   size="small"></Input>
          </FormItem>
          <FormItem label="单位类别编码"
                    prop="sdCode"
                    :rules="[{ required: true, message: '请输入单位类别编码', trigger: 'blur' }]">
  ...
  methods: {
    ...
    save () {
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.isLoading = true
          unitTypeSave(this.formData).then(({ data }) => {
