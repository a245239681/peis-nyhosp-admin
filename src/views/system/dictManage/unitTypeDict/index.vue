<template>
  <div class="unit-type-dict-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">单位类别字典管理</span>
...
<script>
import { unitTypeList, unitTypeStatus, unitTypeDelete } from '@/api/dict_unitType.js'
...
export default {
  ...
  methods: {
    getList (page = 1, size = 20) {
      ...
      this.isLoading = true
      unitTypeList(pagination, params).then(({ data }) => {
    ...
    statusChange (row) {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          unitTypeStatus(params).then(({ data }) => {
            if (data.code == 0) {
              this.$Message.success(`已${msg}该单位类别`)
              this.$store.dispatch('getUnitTypeDict', true)
    ...
    deleteData (row) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          unitTypeDelete(params).then(({ data }) => {
            if (data.code == 0) {
