<template>
  <div class="identity-dict-wrapper">
    <Card style="width:100%;">
      <div slot="title"
           class="search-title-wrapper">
        <span class="title">身份字典管理</span>
...
<script>
import { identifyList, identifyStatus, identifyDelete } from '@/api/dict_identify.js'
...
export default {
  ...
  methods: {
    getList (page = 1, size = 20) {
      ...
      this.isLoading = true
      identifyList(pagination, params).then(({ data }) => {
    ...
    statusChange (row) {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          identifyStatus(params).then(({ data }) => {
            if (data.code == 0) {
              this.$Message.success(`已${msg}该身份`)
    ...
    deleteData (row) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          identifyDelete(params).then(({ data }) => {
            if (data.code == 0) {
